...
(function() {
  var addShape, angle, axes, axis, camera, cube2, directionalLight, directionalLight2, height, matrix, parentMesh, render, renderer, scene, width;
  ...
  addShape = function(threeShape, i) {
    var geometry, test;

    console.log(i);
    geometry = new THREE.ExtrudeGeometry(threeShape, {
      size: 10,
      amount: 1,
      bevelEnabled: false,
      steps: 1
    ...
    return parentMesh.add(test);
  };

  window.addShapes = function(shapes) {
    var i, mapped, point, points, pointsAlreadyMapped, shape, threeShape, um, x, _i, _j, _len, _len1;

    mapped = [];
    for (x = _i = 0, _len = shapes.length; _i < _len; x = ++_i) {
      shape = shapes[x];
      ...
      for (i = _j = 0, _len1 = points.length; _j < _len1; i = ++_j) {
        point = points[i];
        if (isNaN(point[1]) || isNaN(point[2])) {
          continue;
        }
